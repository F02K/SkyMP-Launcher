# Launcher 2.0 backend contract

The backend must add these authenticated-as-needed, server-scoped routes below
...
The existing unscoped routes and `/launcher/servers/default` remain aliases for
the default server while Launcher 1.x is supported. Launcher 2.0 only falls back
to those aliases when a scoped metadata route returns HTTP 404. It never falls
back to an unsigned client package.
...
```json
{
  ...
  "files": [
    { "path": "Data/Platform/Plugins/example.dll", "size": 1234, "sha256": "64 lowercase hex characters" }
  ],
